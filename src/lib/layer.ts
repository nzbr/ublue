...
export abstract class GenericLayer implements Layer {
    ...
    async build(buildContainer: Container): Promise<Directory> {
        ...
        if (this.buildScript) {
            buildDir = buildDir.withNewFile("build.sh", this.buildScript, {
                permissions: 0o755,
            });
    ...
    buildCached(
        ...
        this.buildCache[buildContainerId.toString()] =
            this.build(buildContainer);
    ...
    async install(
        ...
        const buildDir = (
            await this.buildCached(buildContainer, await buildContainer.id())
        ).withNewFile("install.sh", unindent(this.installScript), {
            permissions: 0o755,
        });
...
export abstract class CompositeLayer implements Layer {
    ...
    install(
        ...
        return this.layers.reduce(
            (container, layer) =>
                container.then((result) =>
                    layer.install(buildContainer, result),
                ),
