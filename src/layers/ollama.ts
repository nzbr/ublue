...
export class OllamaLayer extends GenericLayer {
    ...
    async build(buildContainer: Container): Promise<Directory> {
        const baseUrl =
            "https://github.com/ollama/ollama/releases/download/v0.30.11";
        ...
        let outDir = dag.directory().withFile(
            "ollama-linux-amd64.rpm",
            await _mkRPM(
                {
                    name: "ollama",
                    version,
                ...
                dag
                    ...
                    .withDirectory(
                        "usr",
                        _fetchTarball(`${baseUrl}/ollama-linux-amd64.tgz`),
                    ),
            ),
        );

        if (this.withRocm) {
            outDir = outDir.withFile(
                "ollama-rocm-linux-amd64.rpm",
                await _mkRPM(
                    {
                        name: "ollama-rocm",
                        version,
                    ...
                    dag
                        ...
                        .withDirectory(
                            "usr",
                            _fetchTarball(
                                `${baseUrl}/ollama-linux-amd64-rocm.tgz`,
                            ),
                        ),
                ),
            );
